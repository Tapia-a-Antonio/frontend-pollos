...
export const Catalogo = ({ setVistaActual, usuario, agregarACarrito }) => {
  ...
  useEffect(() => {
    const cargaDatosCatalogo = async () => {
      ...
      try {
        ...
        // Solo guardamos el error, NO volvemos a llamar a la función
  ...
  const agregarCarrito = (producto) => {
    // 1. Si no hay usuario en sesión, lo redirigimos directamente a la vista de Login
    if (!usuario) {
      setVistaActual('login');
      return;
    }

    // 2. Si hay usuario, verificamos que su rol sea exactamente 'ROLE_CLIENTE'
    if (usuario.rol !== 'ROLE_CLIENTE') {
      alert('Solo los usuarios registrados como Clientes pueden realizar compras.');
      return;
    }

    // 3. Si pasa ambas validaciones, se agrega el producto exitosamente
  ...
  const filtroProductos = productos.filter((producto) => {
    ...
    const busquedaCategorias =
      ...
      (producto.categorias && producto.categorias.nombre === selecionCategoria); // <-- CORRECCIÓN AQUÍ
        ...
        <div className="w-full md:w-1/4 flex-shrink-0 space-y-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm space-y-3">
            <h3 className="font-bold text-gray-800 flex items-center gap-2 text-sm uppercase tracking-wider">
              <Search className="w-4 h-4 text-indigo-500" /> Buscar Producto
            </h3>
            <div className="relative">
              <input
                ...
                className="w-full p-3 pl-4 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm text-gray-900"
              />
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm space-y-4">
            <h3 className="font-bold text-gray-800 flex items-center gap-2 text-sm uppercase tracking-wider">
              <Filter className="w-4 h-4 text-indigo-500" /> Categorías
            </h3>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="h-48 w-full bg-gray-100 relative overflow-hidden">
                      ...
                      {/* Categoría Badge */}
                      {producto.categorias && (
                        <span className="absolute top-3 left-3 bg-indigo-900/80 text-white text-xs font-bold px-2.5 py-1 rounded-full backdrop-blur-sm">
                    ...
                    <div className="p-4 flex-grow flex flex-col justify-between">
                      {/* Sección de Textos (Título y Descripción) */}
                      <div className="space-y-1">
                        {/* Proveedor */}
                        {producto.proveedor && (
                          <div className="text-xs text-gray-400 font-semibold flex items-center gap-1">
                            <i className="fa-solid fa-truck text-indigo-400"></i> {producto.proveedor.nombreEmpresa}
                          </div>
                        )}
                      ...
                      {/* Precio y Stock */}
                      <div className="pt-3 flex flex-col justify-end mt-auto">
                        {/* Contenedor de Precio y Disponibilidad más compacto */}
                        <div className="flex flex-col items-center gap-1 mb-3">
                          {/* Precio reducido a text-lg */}
                          <span className="font-extrabold text-lg text-indigo-900 text-center leading-none">
                            ${producto.precio.toLocaleString('es-MX', { minimumFractionDigits: 2 })} <span className="text-[10px] font-semibold text-indigo-600">MXN</span>
                          </span>
